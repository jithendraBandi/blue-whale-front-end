    ...
    <div key={index} className="transaction-modal-item-details-container">
        <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
          <Col className="gutter-row" span={20}>
            ...
            >
              <FloatSelect showSearch label="Item" onChange={(value) => {
                changeItem(value);
                getItemTotal();
              }}>
                {itemsList?.map((item) => (
          ...
          <Col className="gutter-row" span={12}>
            <Form.Item
              ...
              rules={[{ required: true, message: "" }]}
            >
              <FloatInput onChange={getItemTotal} type="number" label="Quantity" />
            </Form.Item>
          </Col>
          <Col className="gutter-row" span={12}>
            <Form.Item
        ...
        <span className="flex-end green-color">{itemTotal?.[index] ? `Rs. ${itemTotal?.[index]} /-` : null}</span>
      </div>
  )
}
